...
import { queuePayment, isOnline } from "@/lib/offline-queue";
...
export default function CobradorMapaPage() {
  ...
  useEffect(() => {
    if (status === "unauthenticated") {
      router.push("/login?callbackUrl=/mobile/cobrador/mapa");
    }
  }, [status, router]);
  ...
  const handlePayment = async (method: string) => {
    ...
    if (!isOnline()) {
      try {
        ...
        alert("Pagamento salvo offline! Será sincronizado quando houver internet.");
      } catch (e: any) {
    ...
    <>
      <div className="flex flex-col" style={{ height: "calc(100vh - 64px)" }}>
        ...
        <div className="bg-white border-b border-gray-200 px-4 pt-12 pb-3 flex-shrink-0">
          ...
          {offline && (
            <div className="mt-1 bg-amber-50 border border-amber-200 rounded-lg px-3 py-1 text-amber-700 text-xs font-medium inline-flex items-center gap-1">
